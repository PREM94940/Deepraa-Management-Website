...
export default function CustomCampaignPage() {
    ...
    // Load CMS data dynamically by slug
    const { sections, globalSettings, pageData, loading } = useStorefrontCMS(slug, true);

    if (loading) {
        return (
            <main className="min-h-screen bg-surface flex items-center justify-center">
                <div className="animate-pulse text-xl font-bold italic font-display text-muted">Preparing your luxury campaign...</div>
            </main>
        );
    }
        ...
        return (
            <main className="min-h-screen bg-surface flex flex-col justify-between">
                ...
                <div className="py-32 text-center max-w-xl mx-auto px-6">
                    ...
                    <Link href="/" className="px-8 py-3 bg-black text-white hover:bg-gold transition-colors text-xs font-bold uppercase tracking-widest">
                        Return to Homepage
                    </Link>
                </div>
                    ...
                    sections.map((section, idx) => {
                        ...
                        return (
                            <div key={idx} id={`section-${section.type}`} className={`${visibilityClass} ${spacingClass}`}>
                                <Component data={section.settings} />
                            </div>
                        );
                    })
                ) : (
