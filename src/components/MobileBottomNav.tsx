    ...
    return (
        <nav className="fixed bottom-0 left-0 w-full bg-white/90 backdrop-blur-md border-t border-gray-100 z-50 md:hidden pb-safe">
            <div className="flex justify-around items-center h-16 px-4">
                <Link href="/" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${pathname === '/' ? 'text-black' : 'text-gray-600'}`}>
                    <Home size={22} strokeWidth={pathname === '/' ? 2.5 : 2} />
                    <span className="text-[10px] font-medium tracking-wide">Home</span>
                </Link>

                <Link href="/collections" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${pathname.includes('/collections') ? 'text-black' : 'text-gray-600'}`}>
                ...
                <button 
                    onClick={toggleCart} 
                    className="flex flex-col items-center justify-center w-full h-full space-y-1 text-gray-600 relative"
                >
                ...
                <Link href="/account" className={`flex flex-col items-center justify-center w-full h-full space-y-1 ${pathname.includes('/account') ? 'text-black' : 'text-gray-600'}`}>
