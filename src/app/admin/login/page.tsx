...
import { Mail, Lock, Shield, Eye, EyeOff, ArrowRight, KeyRound, ArrowLeft } from 'lucide-react';
...
export default function AdminLogin() {
    ...
    const [gatekeyVerified, setGatekeyVerified] = useState(false);
    ...
    const handleVerifyGatekeyOnly = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!gatekey) return setError('Please enter the Admin Gatekey.');
        setLoading(true);
        setError(null);
    ...
    return (
        <main className="min-h-screen bg-[#060606] flex flex-col justify-center items-center px-4 py-12 relative overflow-hidden">
            ...
            <Link 
                href="/" 
                className="absolute top-8 left-8 z-20 flex items-center gap-2 text-zinc-500 hover:text-white transition-colors text-xs font-bold uppercase tracking-widest"
            >
            ...
            <div className="w-full max-w-[400px] bg-[#0E0E0E] border border-zinc-800 p-8 rounded-sm shadow-2xl relative z-10">
                ...
                {!gatekeyVerified ? (
                    <form onSubmit={handleVerifyGatekeyOnly} className="space-y-5">
                        <div>
                            <label className="block text-[8px] text-[#A3A3A3] mb-1.5 uppercase font-bold tracking-widest">Security Gateway</label>
                            <p className="text-xs text-zinc-500 mb-4">Please enter the operational gatekey to proceed to staff login.</p>
                            <div className="relative">
                                <KeyRound className="w-4 h-4 absolute left-3.5 top-3.5 text-zinc-500" />
                                <input 
                                    type="password"
                                    placeholder="Enter Gatekey" 
